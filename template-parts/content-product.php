...
<?php 
    $product_data = wc_get_product($args['id']);
    $product_variation = get_product_variations($args['id']);
    if ( $product_data->is_in_stock() ) {
?>
<div class="product-card card" data-product-id="<?= $args['id'] ?>" data-product-name="<?= $args['name'] ?>" data-product-price="<?= $args['price'] ?>">
    ...
    <div class="content">
        <div class="wrapper">
            <div class="upper">
                <p class="title"><?= $product_data->get_name() ?></p>
                <?php if(!empty($product_variation)): ?>
                    <div class="options product-variation">
                        <form>
                            <?php for($i = 0; $i < count($product_variation['ids']); $i++): ?>
                                <div class="option option<?= $i ?>">
                                    <input type="radio" name="option" value="<?= $product_variation['ids'][$i] ?>" id="option_<?= $i ?>" <?php if(count($product_variation['ids']) - $i == 1): echo 'checked'; endif; ?>>
                                    <label><?= $product_variation['labels'][$i] ?></label>
                                </div>
                            <?php endfor; ?>
                        </form>
                    </div>
	        ...
	        <a data-quantity="1" data-product_id="<?= esc_attr( $args['id'] ) ?>" class="btn btn-primary nougatine-add-to-cart product_type_simple custom-add-to-cart">
...
<?php } else { ?>
<div class="product-card card out-of-stock" data-product-id="<?= $args['id'] ?>" data-product-name="<?= $args['name'] ?>" data-product-price="<?= $args['price'] ?>">
    <div class="image-holder">
                ...
                <div class="options">
                    <form>
                        <?php if(!empty($product_variation)): ?>
                            <?php for($i = 0; $i < count($product_variation['ids']); $i++): ?>
                                <div class="option option<?= $i ?>">
                                    <input type="radio" name="option" id="option_<?= $i ?>" <?php if(count($product_variation['ids']) - $i == 1): echo 'checked'; endif; ?>>
                                    <label><?= $product_variation['labels'][$i] ?></label>
                                </div>
                            <?php endfor; ?>
                        <?php else: ?>
